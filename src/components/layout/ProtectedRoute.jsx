...
const ProtectedRoute = ({ allowedRoles }) => {
  ...
  const bypassRejection = location.state?.bypassRejection;

  if (isListingRoute && hasBusiness) {
    if (userRole === "BUSINESS" || userRole === "OWNER") {
      return <Navigate to="/owner-dashboard" replace />;
    } else if (businessStatus === "REJECTED" && bypassRejection) {
      // Let the user proceed to the listing form to re-apply
    } else {
      // Force them to the pending/rejected screen
  ...
  const isBusinessRoute =
    allowedRoles &&
    (allowedRoles.includes("BUSINESS") || allowedRoles.includes("OWNER"));
