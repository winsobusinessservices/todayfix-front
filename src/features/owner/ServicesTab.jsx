...
const INITIAL_SERVICES = [
  { id: 1, name: 'Deep Home Cleaning', price: '4500', active: true, description: 'Complete deep cleaning of 3BHK including bathrooms and kitchen.' },
  { id: 2, name: 'AC Servicing & Repair', price: '1200', active: true, description: 'Standard jet cleaning and gas check for split ACs.' },
  { id: 3, name: 'Sofa Dry Cleaning', price: '800', active: false, description: 'Per seat dry cleaning with industrial vacuum.' },
];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-text-primary mb-2">My Services</h1>
          <p className="text-zinc-400">Manage what you offer and your pricing.</p>
        </div>
        ...
        {services.map((service) => (
          <div 
            ...
            className={`bg-surface-primary rounded-3xl border p-6 shadow-2xl shadow-black/5 transition-all duration-300 ${
              service.active ? 'border-border-primary' : 'border-border-primary opacity-60 grayscale'
            }`}
          >
            <div className="flex justify-between items-start mb-4">
              ...
              <ToggleSwitch active={service.active} onToggle={() => toggleService(service.id)} />
            </div>

            <h3 className="text-xl font-bold tracking-tight text-text-primary mb-2">{service.name}</h3>
            <p className="text-sm text-zinc-400 line-clamp-2 mb-6">{service.description}</p>
          ...
          <>
            ...
            >
              <div className="p-6 border-b border-border-primary flex justify-between items-center">
                ...
                <button onClick={() => setIsModalOpen(false)} className="text-zinc-400 hover:text-text-primary"><X size={24} /></button>
              </div>
              <form onSubmit={handleSave} className="p-6 space-y-4">
                <div>
                  <label className="block text-sm font-bold text-text-primary mb-1">Service Name</label>
                  <input 
                    required
                    type="text" 
                    value={formData.name}
                ...
                <div className="pt-4 flex gap-3">
                  <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 py-3 bg-surface-secondary text-text-primary font-bold rounded-xl hover:bg-border-primary transition-colors">Cancel</button>
                  <button type="submit" className="flex-1 py-3 bg-surface-dark text-text-inverted font-bold rounded-xl hover:scale-[0.98] transition-transform shadow-md">Save Service</button>
                </div>
              </form>
            </motion.div>
          </>
