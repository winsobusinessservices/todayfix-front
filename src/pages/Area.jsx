...
const Area = () => {
  ...
  const areaInfo = {
    ...
    city: "Bengaluru",
    pincode: "560038",
    heroImage: "https://images.unsplash.com/photo-1517713982677-4b66332f98de?ixlib=rb-4.0.3&auto=format&fit=crop&w=1920&q=80", // Premium neighborhood/cafe street vibe
    stats: {
      activePros: 342,
  ...
  const categories = ["All", "Home Cleaning", "Plumbing", "Interior Design", "Electricians", "Appliance Repair", "Packers & Movers"];

  const localVendors = [
    {
      id: 1,
      name: "Aura Spaces Studio",
      avatar: "https://api.dicebear.com/7.x/shapes/svg?seed=Aura&backgroundColor=0284c7",
    ...
    },
    {
      ...
      avatar: "https://api.dicebear.com/7.x/shapes/svg?seed=QuickFix&backgroundColor=ea580c",
      category: "Plumbing",
      rating: 4.7,
      reviews: 412,
    ...
    },
    {
      ...
      avatar: "https://api.dicebear.com/7.x/shapes/svg?seed=Sparkle&backgroundColor=059669",
    ...
    },
    {
      id: 4,
      name: "Volt Experts",
      avatar: "https://api.dicebear.com/7.x/shapes/svg?seed=Volt&backgroundColor=7c3aed",
    ...
    <div className="min-h-screen bg-surface-secondary font-sans">
      ...
      <div className="bg-surface-primary border-b border-border-primary py-3 px-6">
        <div className="max-w-6xl mx-auto flex items-center text-sm font-medium text-text-secondary gap-2">
          <Link to="#" className="hover:text-text-primary hover:underline transition-colors">Home</Link>
          <svg className="w-4 h-4 text-text-muted" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clipRule="evenodd" /></svg>
          <Link to="#" className="hover:text-text-primary hover:underline transition-colors">{areaInfo.city}</Link>
          <svg className="w-4 h-4 text-text-muted" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clipRule="evenodd" /></svg>
          ...
          <div className="max-w-6xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <div className="max-w-2xl">
              ...
              <p className="text-text-muted text-lg font-medium leading-relaxed">
                Connect with {areaInfo.stats.activePros}+ verified professionals actively serving the {areaInfo.name} neighborhood.
              </p>
            </div>
            ...
            <div className="hidden lg:flex gap-6 bg-surface-dark/60 backdrop-blur-xl border border-border-dark p-5 rounded-2xl">
              <div>
                <p className="text-2xl font-extrabold text-white">{areaInfo.stats.avgResponseTime}</p>
                <p className="text-xs text-text-muted font-bold uppercase tracking-wider mt-1">Avg Response</p>
              </div>
              <div className="w-px bg-slate-600"></div>
              <div>
                <p className="text-2xl font-extrabold text-emerald-400">{areaInfo.stats.jobsCompleted}</p>
                <p className="text-xs text-text-muted font-bold uppercase tracking-wider mt-1">Jobs Done Here</p>
              </div>
            </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
            ...
            >
              ...
              <div className="flex flex-col items-center gap-3 sm:w-28 flex-shrink-0">
                <div className="relative w-24 h-24 rounded-2xl overflow-hidden bg-surface-secondary border border-border-secondary shadow-[0_4px_20px_rgb(0,0,0,0.03)] group-hover:scale-105 transition-transform">
                  <img src={vendor.avatar} alt={vendor.name} className="w-full h-full object-cover p-2" />
                </div>
              ...
              <div className="flex-grow flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-sm font-semibold text-text-secondary mb-3">{vendor.category}</p>
                  ...
                  <div className="flex items-center gap-1.5 mb-4">
                    ...
                    <span className="font-bold text-text-primary text-sm">{vendor.rating}</span>
                    <span className="text-sm text-text-muted">({vendor.reviews} reviews)</span>
                  </div>

                  {/* Tags */}
                  <div className="flex flex-wrap gap-2 mb-4 sm:mb-0">
                    {vendor.tags.map((tag, idx) => (
                      <span key={idx} className="bg-surface-secondary text-text-secondary px-2.5 py-1 rounded-md text-[11px] font-semibold uppercase tracking-wider">
                        {tag}
                      </span>
                    ))}
                  </div>
                ...
                <div className="flex items-center gap-2 mt-4 pt-4 border-t border-border-secondary">
                  ...
                  <button className="flex-1 bg-surface-primary hover:bg-surface-secondary border border-border-primary text-text-primary py-2.5 rounded-xl text-sm font-semibold transition-colors active:scale-95">
                    View Profile
                  </button>
                </div>
              </div>
        ...
        <div className="mt-16 bg-surface-secondary/50 rounded-3xl p-8 md:p-12 border border-border-primary">
          ...
          <p className="text-text-secondary leading-relaxed mb-6">
            Hiring service providers near {areaInfo.name}, {areaInfo.city} ensures faster response times, lower travel costs, and deep knowledge of the local area's specific requirements (like building society rules and traffic timings). Our verified vendors in the {areaInfo.pincode} area have completed thousands of successful jobs with top ratings from your neighbors.
          </p>
