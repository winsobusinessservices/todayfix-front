...
const SIDEBAR_SECTIONS = [
  {
    label: "Main",
    items: [{ id: "", label: "Overview", icon: LayoutDashboard }],
  },
  {
    label: "Management",
    items: [
...
const AdminDashboard = () => {
  ...
  return (
    <div className="bg-surface-secondary h-screen flex overflow-hidden font-sans">
            ...
            >
              <div className="p-4 border-b border-border-primary flex items-center gap-3">
                ...
                <div className="flex items-center gap-1">
                  <kbd className="px-2 py-1 bg-surface-secondary rounded border border-border-primary text-xs font-bold text-zinc-500 font-mono">
                    ESC
                  </kbd>
                </div>
              </div>
              <div className="p-4 bg-surface-secondary/30 h-64 overflow-y-auto styled-scrollbar">
                <p className="text-sm font-bold text-zinc-500 mb-2 px-2 uppercase tracking-wider">
                  Recent Searches
                </p>
      ...
      {/* Left Sidebar */}
      <aside
        className={`w-72 bg-surface-primary border-r border-border-primary flex flex-col z-50 shadow-2xl lg:shadow-none shrink-0 fixed inset-y-0 left-0 transform transition-transform duration-300 lg:relative lg:translate-x-0 ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        ...
        <nav className="flex-1 overflow-y-auto styled-scrollbar py-6 px-4 space-y-8">
          {SIDEBAR_SECTIONS.map((section, sIdx) => (
            <div key={sIdx}>
              <p className="px-3 mb-3 text-[11px] font-black text-zinc-400 uppercase tracking-widest">
                {section.label}
              </p>
              <div className="space-y-1">
        ...
        <div className="p-4 border-t border-border-primary shrink-0">
          <button
            ...
            className="flex items-center justify-center gap-2 px-4 py-3 w-full text-zinc-500 hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-colors duration-200 font-bold text-sm cursor-pointer"
          >
            <LogOut size={18} />
            Log Out
          </button>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-surface-secondary">
        ...
        <header className="bg-surface-primary border-b border-border-primary h-20 shrink-0 px-4 md:px-8 flex items-center justify-between z-10 sticky top-0">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              className="md:hidden w-10 h-10 rounded-full flex items-center justify-center text-zinc-500 hover:bg-surface-secondary"
              onClick={() => setIsSearchOpen(true)}
            >
              <Search size={20} />
            </button>
