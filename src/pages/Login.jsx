...
const Login = () => {
  ...
  const handleLoginSuccess = async (result) => {
    if (result.success && result.data && result.data.access) {
      try {
        const statusResponse = await getAccountDeletionStatus(
          result.data.access,
        );
        if (statusResponse.data?.deletion_requested) {
        ...
        // Fallback to normal login if checking status fails
        finishLogin(result);
      }
    }
  };
  ...
  const handleRestoreAccount = async () => {
    ...
    setIsRestoring(true);
    try {
      await cancelAccountDeletion(pendingLoginResult.data.access);
      toast.success("Account restoration successful!");
      setShowRestoreModal(false);
      finishLogin(pendingLoginResult);
    } catch (err) {
  ...
  const handleCancelRestore = () => {
    setShowRestoreModal(false);
    setPendingLoginResult(null);
    popup("Info", "Login cancelled.", "info");
  };
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm border">
            ...
            >
              ...
              <p className="text-text-secondary text-sm mb-6 leading-relaxed">
                You have previously requested to delete your account. Do you
                want to cancel the deletion request and restore your account?
              </p>
              <div className="flex gap-3">
                <button
                  onClick={handleCancelRestore}
                  disabled={isRestoring}
                  className="flex-1 px-4 py-2.5 bg-surface-secondary text-text-primary font-bold rounded-xl hover:bg-zinc-200 transition-colors cursor-pointer disabled:opacity-50"
                >
                  No, Cancel
                </button>
                <button
                  ...
                  disabled={isRestoring}
                  className="flex-1 px-4 py-2.5 btn-primary bg-text-primary text-surface-primary font-bold rounded-xl hover:opacity-90 transition-opacity shadow-lg cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2"
                >
